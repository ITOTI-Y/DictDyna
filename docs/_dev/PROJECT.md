...
### Phase 5 消融实验（2026-03-24）

#### A 组：组件消融（单建筑 hot，3 episodes）

**单 seed（seed=42）**：

| 变体 | Ep1 | Ep2 | Ep3 | vs Baseline |
|------|-----|-----|-----|-------------|
| **Baseline** (Dyna-SAC, pretrained D, dict_lr=1e-5) | -6901 | -5518 | **-5455** | — |
| Random Dict (无预训练) | -7013 | -5533 | -5428 | +0.5% |
| Fixed Dict (dict_lr=0, 冻结) | -10420 | -11405 | -8800 | **-61.3%** |
| No Rollout (rollout_start=999999) | -7227 | -5705 | -5547 | -1.7% |
| SAC Only (无 WM) | -7234 | -6099 | -5575 | -2.2% |

**3-seed 验证（关键消融，Ep3 mean±std）**：

| 变体 | s42 | s123 | s7 | Mean±Std | vs Baseline |
|------|-----|------|-----|----------|-------------|
| **Baseline** | -5455 | -5274 | -4994 | **-5241±194** | — |
| Fixed Dict | -8800 | -18860 | -5960 | -11207±5573 | **-114%（高方差，训练不稳定）** |
| K=256 | -5266 | -5525 | -5565 | -5452±131 | -4.0%（优势未复现） |

**发现**：
- **字典微调是关键**：Fixed Dict 均值 -11207，训练极不稳定（std=5573），确认 slow_update_lr 不可或缺
- **预训练 D 贡献微弱**：Random vs Pretrained 几乎无差（+0.5%），D 的价值在微调而非预训练
- **K=256 优势不显著**：3-seed 验证未复现 seed=42 的 +3.5%，K=128 是合理默认值
- **Rollout 贡献微弱**：No Rollout 仅差 -1.7%，但 Ep1 有 +4.5% sample efficiency 增益

#### B 组：超参数消融（seed=42）

| 变体 | Ep1 | Ep2 | Ep3 | 稀疏性 | vs Baseline |
|------|-----|-----|-----|--------|-------------|
| K=64 | -7346 | -6046 | -5543 | 87.5% | -1.6% |
| **K=128** | -6901 | -5518 | **-5455** | 87.5% | — |
| K=256 | -6853 | -5472 | -5266 | 93.75% | +3.5%（3-seed 未复现） |
| topk=8 | -7110 | -5801 | -5826 | 93.75% | -6.8% |
| **topk=16** | -6901 | -5518 | **-5455** | 87.5% | — |
| topk=32 | -7009 | -5507 | **-5292** | 75% | +3.0% |

**发现**：
- **K=128 是合理默认值**：K=64 略差，K=256 无显著优势
- 过度稀疏（topk=8, 93.75%）有害 (-6.8%)，适度降低稀疏度（topk=32）有微小增益 (+3.0%)

#### C 组：Context Dim 消融（Transfer，seed=42）

| context_dim | 1d Adv | 3d Adv | 7d Adv | 均值 |
|------------|--------|--------|--------|------|
| 4 | +31.5% | +55.6% | +42.1% | +43.1% |
| 8 | +61.6% | +49.7% | +61.5% | +57.6% |
| **16** | +56.9% | **+65.5%** | +40.9% | +54.4% |
| 32 | +53.5% | +52.2% | +49.8% | +51.8% |

**发现**：
- dim=8 和 dim=16 表现最优，均值接近（57.6% vs 54.4%）
- dim=4 太小（1d 仅 +31.5%），信息瓶颈
- dim=32 无额外收益，可能过参数化
