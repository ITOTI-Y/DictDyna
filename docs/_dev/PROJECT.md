...
#### A 组：组件消融（单建筑 hot，3 episodes，seed=42）
...
**发现**：
- **字典微调是关键**：Fixed Dict（冻结 D）性能断崖式下降 -61%，说明 slow_update_lr 是核心组件
- **预训练 D 贡献微弱**：Random vs Pretrained 几乎无差（+0.5%），D 的价值主要在微调中体现
- **Rollout 贡献微弱**：No Rollout 仅差 -1.7%，接近 SAC Only 的 -2.2%。说明在充分训练（3 episodes）后 rollout 边际贡献小
- **Rollout 在 Ep1 有优势**：Baseline Ep1=-6901 vs No Rollout -7227 (+4.5%)，sample efficiency 增益
...
| K=256 | -6853 | -5472 | **-5266** | 93.75% | **+3.5%** |
| topk=8 | -7110 | -5801 | -5826 | 93.75% | -6.8% |
| **topk=16** | -6901 | -5518 | **-5455** | 87.5% | — |
| topk=32 | -7009 | -5507 | **-5292** | 75% | **+3.0%** |

**发现**：
- **K=256 最优**：更多原子 → 更好性能（+3.5%），表达力饱和尚未到达
- **topk=32 最优**：更多活跃原子 → 更好（+3.0%），过度稀疏（topk=8）有害 (-6.8%)
- 最优组合可能是 K=256 + topk=32，但当前实验未测试
